...
class ReportsController extends Controller
{
	...
	public function asanaTask(string $profile = "my", Request $request) {
		...
		$id = table::asana_users()->whereNotNull('reference')->orderBy('reference')->first()->reference;
		...
		if($profile == "my"){
			...
			table::reportviews()->where('report_id', 8)->update(['last_viewed' => $today]);
			...
			table::reportviews()->where('report_id', 9)->update(['last_viewed' => $today]);
        ...
        if($profile=='department'){
            $personal_name = "Department";
            $parent_name = "Other Departments";
            if(isset($parent))
				$parent = 'Other departments in same company ('.ucwords(strtolower($parent)).')';
        }
        else{ 
        	$personal_name = "$name";
            $parent_name = "$name Teammates";
            if(isset($parent))
				$parent = "Other staff in $name department (".ucwords(strtolower($parent)).')';
        }

        if(\Auth::user()->role_id == 2){
 			$others = array();
 			if($profile=='department'){
 				$others_list = table::department()->select('tbl_form_department.id as value','tbl_form_department.department as name')->orderBy('tbl_form_department.department')->get()->toArray();
 			}
			else {
				$others_list = table::asana_users()->select('tbl_asana_users.reference as value', DB::raw('concat(tbl_people.firstname, " ", tbl_people.lastname) as name'))->leftjoin('tbl_people','tbl_asana_users.reference','=','tbl_people.id')->whereNotNull('tbl_people.firstname')->whereNotNull('reference')->orderBy('tbl_people.firstname')->get()->toArray();
			}

			foreach ($others_list as $value) {
	        	$others[$value->value] = $value->name;
	        }
 		}

		return view('admin.reports.report-asana-task', 
			compact(
				'others', 'mine',
