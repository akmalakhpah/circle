    ...
    public function add(Request $request)
    {
        ...
        if ($request->idno == null || $request->type == null ) {
            return response()->json([
                "error" => "Whoops! Please provide your ID number and click Time In or Out!"
            ]);
        }
        ...
        if ($clock_comment == 1) {
            if ($request->clock_comment == null ) {
                return response()->json([
                    "error" => "Whoops! Please provide your comment!"
                ]);
            }
        }
        ...
        if ($iprestriction != null) {
            ...
            if(in_array($ip, $ips) == false) {
                $msge = "Whoops! You are not allowed to Clock-In/Out from your IP address ".$ip;
                return response()->json([
                    "error" => $msge,
                ]);
            }
        ...
        if ($type == 'Time In') {
            ...
            if ($has == 1) {
                ...
                $hti = date('h:i:s A', strtotime($hti));
                return response()->json([
                    "employee" => $employee,
                    "gender" => $gender,
        ...
        if ($type == 'Time Out') {
            ...
            if($timeIN == null) {
                return response()->json([
                    "error" => "Please Time-In before Time-Out."
                ]);
            } 

            if ($hasout != null) {
                $hto = table::attendance()->where([['idno', $idno],['date', $date]])->value('timeout');
                $hto = date('h:i:s A', strtotime($hto));
                return response()->json([
                    "employee" => $employee,
                    "gender" => $gender,
                    "civilstatus" => $civilstatus,
