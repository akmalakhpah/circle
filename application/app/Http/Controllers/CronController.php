        ...
        switch ($type) {
            ...
            case "tasks":
                ...
                foreach ($projects as $proj_gid) {
                    $tasks = $client->tasks->findByProject($proj_gid->gid,array(),array('opt_fields' => 'id,name,assignee.id,completed,completed_at,created_at,due_at'));
                    foreach ($tasks as $task) {
                        ...
                        if(isset($task->due_at))
                            $due_at = date('Y-m-d h:i:s', strtotime($task->due_at));
            ...
            case "tasklists":
                ...
                foreach ($users as $task_list) {
                    if(isset($task_list->user_task_list_gid)){
                        ...
                        $tasks = $client->getCollection($path, array('opt_fields' => 'id,name,assignee.id,completed,completed_at,projects.id,created_at,due_at', 'completed_since' => $date_limit));
