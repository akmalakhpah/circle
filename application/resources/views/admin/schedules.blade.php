    ...
    <div class="container-fluid">
        <div class="row">
            <h2 class="page-title">Schedules
                <button class="ui positivex blue button mini offsettop5 btn-add float-right"><i class="ui icon plus"></i>Add</button>
            </h2>
        </div>
                    ...
                    <table width="100%" class="table table-striped table-hover" id="dataTables-example" data-order='[[ 7, "desc" ]]'>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ $sched->restday }}</td>
                                <td>@php $datefrom=date('D, M d Y',strtotime($sched->datefrom)); @endphp 
                                    {{ $datefrom }}
                                </td>
                                <td>@php $dateto=date('D, M d Y',strtotime($sched->dateto)); @endphp 
                                    {{ $dateto }}
                                </td>
                                <td>
                                    @if($sched->archive == '0') 
